...
| Martin Fowler (Monolith First, Strangler Fig) | Modular monolith first; extract later | Contract-first boundaries and NATS eventing are extraction-friendly                          |
...
| --- | --------------------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Circuit Breaker       | ✅ Yes                      | Wrap all outbound calls (notification service, webhooks, external rule evaluators) with circuit breaker using nestjs-resilience or opossum. <br> • Prevents cascade failures.                                                                                                                             |
| 2   | Saga Pattern          | ✅ Yes (Choreography-based) | A workflow transition involves: (a) update instance, (b) write audit log, \(c) send notification. If step \(c) fails, we don't rollback (a) and (b). <br> • Use Saga to handle compensation — emit event, notification service retries independently.                                                       |
| 3   | Strangler Fig         | ✅ Yes (Future)             | Start as modular monolith. As WorkflowExecutionModule becomes the bottleneck, extract it into its own service without touching other modules. <br> • The API Gateway routes to the new service transparently.                                                                                             |
...
| 5   | Aggregator Pattern    | ✅ Yes                      | The Instance Detail view needs data from: instance (Execution Module) + audit logs (Audit Module) + user names (Auth Module). <br> • A BFF (Backend for Frontend) aggregator assembles this before sending to client.                                                                                     |
| 6   | API Gateway Pattern   | ✅ Yes                      | Single entry point for all client traffic. Handles JWT validation, tenant extraction, rate limiting, request routing. <br> • Use AWS API Gateway or Kong in production.                                                                                                                                   |
...
| 8   | CQRS Pattern          | ✅ Yes                      | Separate read and write models. Write operations go through the Execution Engine (command side, strongly consistent). <br> • Read operations (list instances, audit history) hit optimized read models / replicas (query side).                                                                           |
...
| 11  | Event Sourcing        | ✅ Partial                  | The audit_logs table is essentially an event log — every state change is a stored event. For full event sourcing, the current state of an instance would be recomputed by replaying audit events. <br> • We implement a hybrid: store current state for fast reads, but audit log is the source of truth. |
...
| 16  | Polyglot Architecture | ✅ Yes                      | NestJS (TypeScript) for all services; PostgreSQL for relational data; MongoDB for Audit Log and Chat History; Redis for caching; NATS for messaging; KAFKA for event store. <br> • Each tool chosen for what it's best at — not one tech for everything.                                                                                                                 |
