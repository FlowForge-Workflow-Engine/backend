...
@Injectable()
export class WorkflowDefinitionPublisher implements OnModuleInit {
  ...
  onModuleInit() {
    this.logger.log(
      "WorkflowDefinitionPublisher initialized — will publish NATS events for workflow definition domain"
    );
  }

  publishWorkflowDefinitionPublished(payload: IWorkflowDefinitionPublishedEvent): void {
    try {
      this.nats.publish(NatsEvents.WORKFLOW_DEFINITION_PUBLISHED, this.codec.encode(payload));
      this.logger.log(
  ...
  publishWorkflowDefinitionDeprecated(payload: IWorkflowDefinitionDeprecatedEvent): void {
    try {
      this.nats.publish(NatsEvents.WORKFLOW_DEFINITION_DEPRECATED, this.codec.encode(payload));
      this.logger.log(
        `Published ${NatsEvents.WORKFLOW_DEFINITION_DEPRECATED} for definition ${payload.definitionId}`
      );
    } catch (err: unknown) {
      this.logger.error("Failed to publish WORKFLOW_DEFINITION_DEPRECATED", err);
    }
  }
}
